...
// Encode encode
func Encode(plainText, secretKey, ivAes []byte) (cipherText []byte, err error) {
	if len(secretKey) != 16 && len(secretKey) != 24 && len(secretKey) != 32 {
		return nil, errors.New("a sixteen or twenty-four or thirty-two length secret key is required")
	}
	...
	if len(ivAes) != 0 {
		if len(ivAes) != block.BlockSize() {
			return nil, errors.New("a sixteen-length ivaes is required")
		} else {
			iv = ivAes
		}
	} else {
		iv = []byte("wumansgy12345678")
...
// Decode decode
func Decode(cipherText, secretKey, ivAes []byte) (plainText []byte, err error) {
	if len(secretKey) != 16 && len(secretKey) != 24 && len(secretKey) != 32 {
		return nil, errors.New("a sixteen or twenty-four or thirty-two length secret key is required")
	}
	block, err := aes.NewCipher(secretKey)
...
func AesCbcEncryptBase64(plainText, secretKey, ivAes []byte) (cipherTextBase64 string, err error) {
	encryBytes, err := Encode(plainText, secretKey, ivAes)
...
func AesCbcDecryptByBase64(cipherTextBase64 string, secretKey, ivAes []byte) (plainText []byte, err error) {
	...
	return Decode(plainTextBytes, secretKey, ivAes)
}
